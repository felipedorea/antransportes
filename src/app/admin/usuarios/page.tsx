...
const emptyForm = { nome: "", cargo: "", descricao: "", foto_url: "" };
...
export default function PilotsPage() {
    ...
    const openEdit = (p: Pilot) => {
        setFormData({ nome: p.nome, cargo: p.cargo, descricao: p.descricao || "", foto_url: p.foto_url || "" });
        setEditingId(p.id);
        setModalMode("edit");
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl p-8 relative max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <input value={formData.descricao} onChange={e => setFormData({ ...formData, descricao: e.target.value })} type="text" placeholder="Descrição breve do cargo (Opcional)" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500/20" />
                            ...
                            <div className="space-y-3">
                                ...
                                {imageMode === "url" ? (
                                    <input value={formData.foto_url} onChange={e => { setFormData({ ...formData, foto_url: e.target.value }); setPreviewUrl(e.target.value || null); }} type="url" placeholder="Cole a URL da imagem aqui" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-500/20" />
                                ) : (
                                    <div
                                ...
                                {(previewUrl || formData.foto_url) && (
                                    <div className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                        <div className="w-14 h-14 rounded-xl overflow-hidden relative shrink-0 bg-slate-200">
                                            <Image src={previewUrl || formData.foto_url || DEFAULT_PILOT_IMG} alt="Preview" fill className="object-cover" />
                                        </div>
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div className="relative">
                                        <div className="w-20 h-20 rounded-2xl bg-slate-100 overflow-hidden border-2 border-slate-50 relative group-hover:border-orange-200 transition-colors">
                                            <Image src={pilot.foto_url || DEFAULT_PILOT_IMG} alt={pilot.nome} fill className="object-cover" />
                                        </div>
                                        {pilot.ativo && (
